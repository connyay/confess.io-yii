...
class FounNavBar extends FounBaseMenu {
    ...
    public function renderItems( $items ) {
        foreach( $items as $item ) {
            if( is_string( $item) && !isset($item["hasDropdown"]) ) {                
                echo "<li>".$item."</li>";
            } else {
    ...
    protected function renderItem( $item ) {
        ...
        if( isset( $item["submenu"] ) ) {
            $dropDownString = '';
            $dropDownString = '<li class="divider"></li><li class="has-dropdown">'."\n"
            . CHtml::link( $item['label'], $item['url'])
            . '<ul class="dropdown">';
            foreach( $item["submenu"] as $subItem ) {
                if( isset( $subItem["divider"] )) {
                    $dropDownString .= '<li class="divider"></li>';
                }
                ...
                $dropDownString .= '<li>';
                $dropDownString .= CHtml::link( $subItem['label'], $subItem['url']);
                $dropDownString .= '</li>';
                
            }
            $dropDownString .= '</ul></li>';
            echo $dropDownString; 
            return;        
        }

        if( isset( $item["flyout"] ) && !isset( $item["submenu"]) && !isset($item["hasDropdown"])) {
